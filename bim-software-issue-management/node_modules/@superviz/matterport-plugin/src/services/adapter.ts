...
import type AblyRealtimeService from '@superviz/sdk/lib/services/realtime/ably';
import type { AblyParticipant } from '@superviz/sdk/lib/services/realtime/ably/types';
import { isEqual } from 'lodash';
import { Vector3, Quaternion } from 'three';

import { Avatar, AvatarsConstants, Name } from '../common/types/avatars.types';
import { Coordinates, DefaultCoordinates } from '../common/types/coordinates.types';
...
export class Presence3D {
  ...
  public attach = (params: DefaultAttachComponentOptions): void => {
    ...
    const { realtime, eventBus, useStore } = params;
    const { localParticipant } = useStore.call(this, 'global-store');
  ...
  private destroy = (): void => {
    ...
    this.realtime.leavePresence3DChannel();
  ...
  private start = (tryingAgain?: boolean): void => {
    if (!tryingAgain) {
      this.realtime.enterPresence3DChannel(this.localParticipant);
    }

    if (!this.realtime.isJoinedRoom || !this.realtime.isJoinedPresence3D) {
  ...
  private subscribeToRealtimeEvents = (): void => {
    ...
    this.realtime.presence3dObserver.subscribe(this.onParticipantsUpdated);
    this.realtime.presence3dJoinedObserver.subscribe(this.onParticipantJoined);
    this.realtime.presence3dLeaveObserver.subscribe(this.onParticipantLeave);
    this.realtime.roomInfoUpdatedObserver.subscribe(this.onRoomInfoUpdate);
  };
  ...
  private unsubscribeToRealtimeEvents = (): void => {
    ...
    this.realtime.presence3dLeaveObserver.unsubscribe(this.onParticipantLeave);
    this.realtime.presence3dObserver.unsubscribe(this.onParticipantsUpdated);
    this.realtime.roomInfoUpdatedObserver.unsubscribe(this.onRoomInfoUpdate);
  };
  ...
  private onParticipantJoined = (participant): void => {
    ...
    const { id, name, avatar, avatarConfig, type } = participant.data;
  ...
  private onLocalParticipantJoined = (participant): void => {
    if (this.config.avatarConfig) {
      this.realtime.setParticipantData({ avatarConfig: this.config.avatarConfig });
    }
  ...
  private onParticipantLeave = (participant): void => {
    this.logger.log('matterport component @ onParticipantLeave', participant);

    const participantToRemove = this.participants.find(
      (participantOnlist) => participantOnlist.id === participant.clientId,
    );
  ...
  private onParticipantUpdated = (participant): void => {
    ...
    const { id, name, avatar, avatarConfig, position, rotation, type } =
  ...
  private onRoomInfoUpdate = (room): void => {
    this.logger.log('matterport component @ onRoomInfoUpdate', room);
    ...
    if (gather) {
      this.moveToAnotherParticipant(hostClientId);
    }

    // Disable gather in next update
  ...
  private createParticipantList = () => {
    const list = this.realtime.getParticipants;

    Object.values(list).forEach((participant: AblyParticipant) => {
      const { data } = participant as any;
  ...
  private createParticipantOn3D = ({ id, name, avatar, avatarConfig, type }): ParticipantOn3D => {
    const participant = {
      id,
      name,
      avatar,
  ...
  private removeParticipant = (participant: ParticipantOn3D, unsubscribe: boolean): void => {
    ...
    if (unsubscribe) {
      this.realtime.unsubscribeFromParticipant3DUpdate(participant.id, this.onParticipantUpdated);
    }
  };
  ...
  private addParticipant = async (participant): Promise<void> => {
    ...
    this.realtime.subscribeToParticipant3DUpdate(participantOn3D.id, this.onParticipantUpdated);
  ...
  private setPrivate = (isPrivate: boolean): void => {
    this.logger.log('matterport component @ private mode', isPrivate);

    this.realtime.updatePresence3D({
  ...
  private async createName(participant: ParticipantOn3D, avatarModel) {
    ...
    const participantSlot: number = this.realtime.getParticipantSlot(participant.id);
    ...
    nameInstance.createName(avatarModel.obj3D, participant.name, participantSlot, nameHeight);
  }
  ...
  private adjustMyPositionToCircle = (position): Coordinates => {
    ...
    this.localSlot = this.realtime.getParticipantSlot(this.localParticipantId);
  ...
  private onParticipantsUpdated = (participants: AblyParticipant) => {
    if (!this.isAttached) return;

    this.logger.log('matterport component @ onParticipantsUpdated', participants);

    Object.values(participants).forEach((participant) => {
      if (participant.clientId === this.localParticipantId) return;
      const participantId = participant.clientId;
      const { position, rotation, sweep, floor, mode, isPrivate } = participant.data;
      ...
      if (isPrivate && this.avatars[participantId]) {
        this.removeParticipant(participant.data, true);
      }
      ...
      if (this.avatars[participantId]) {
        ...
        const laserDestinationPosition: Coordinates = participant.data.laser;
  ...
  private updateAvatar(remoteAvatar: Avatar, position: Coordinates, rotation: Coordinates) {
    if (!this.isAttached) {
      return;
    }
  ...
  private updateLaser(
    ...
    if (!remoteAvatar || !remoteLaser || !laserDestinationPosition || !this.isAttached) {
      return;
    }
    ...
    if (laserInstance) {
      const userSlot = this.realtime.getParticipantSlot(userId);
