...
const LaserPointer = function () {
  ...
  this.onTick = function (tickDelta) {
    ...
    if (this.lastIndex !== this.index) {
      const pointerColor = Colors[this.index];
      if (this.material) {
  ...
  this.createMaterial = (colorIndex) => {
    this.createdMaterial = true;

    const canvas = this.createTextureCanvas();
    const texture = new this.THREE.Texture(canvas);
    texture.needsUpdate = true;
    let colorIndexToSet = 0;
    if (colorIndex !== -1) {
      colorIndexToSet = colorIndex;
    }
    const pointerColor = Colors[colorIndexToSet];
  ...
  this.updateGeometry = (startPos, destPos, isOn, shouldRenderAvatars, index, quat) => {
    this._doUpdateGeometry(isOn, shouldRenderAvatars, index, quat, startPos, destPos);
  };
