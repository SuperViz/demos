...
export class Mouse {
  ...
  public async load(name: string, colorIndex: number): Promise<THREE.Object3D> {
    if (!name || colorIndex === undefined) return;

    return new Promise(() => {
      const svg = `https://production.storage.superviz.com/static/pointers-v2/${colorIndex}.svg`;
      ...
      nameEl.style.color = TextColors[colorIndex].toString();
  ...
  public update = (destPos: THREE.Vector3, colorIndex: number = 0) => {
